...
- `ReviewPageBO<T> { List<T> list; long total; int page; int pageSize; }`：分页结果的通用容器（应用层，不下沉到领域层），字段与 `anitrack-web-rules.md` 已定义的 `PageResponse<T>` 一一对应
...
```java
PageResponse<T> { Integer pageNum; Integer pageSize; Long total; List<T> list; }
```

字段形状直接采用 `docs/rules/anitrack-web-rules.md` 第10.5节已定义的项目统一分页响应约定，不另起一套——本次是该约定在项目中的首次落地实现。
