# anitrack 前端（webui）实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在仓库根目录新建 `webui/`，用 UmiJS Max + Ant Design 实现覆盖 anitrack 后端全部 14 个接口的操作界面。
...
- 项目位置：仓库根目录下 `webui/`，独立 npm 工程，不属于 Maven 多模块
...
- Create: `webui/.umirc.ts`
- Create: `webui/.gitignore`
- Create: `webui/src/layouts/index.tsx`（占位）
- Create: `webui/src/wrappers/auth.tsx`（占位）
- Create: `webui/src/pages/Login/index.tsx`（占位）
- Create: `webui/src/pages/Register/index.tsx`（占位）
- Create: `webui/src/pages/AnimeSearch/index.tsx`（占位）
- Create: `webui/src/pages/AnimeDetail/index.tsx`（占位）
- Create: `webui/src/pages/Watchlist/index.tsx`（占位）
- Create: `webui/src/pages/MyReviews/index.tsx`（占位）
...
Expected: 退出码为 0，输出中不包含 `ERROR`，生成 `webui/dist/` 目录

- [ ] **Step 9: 提交**

```bash
cd webui && cd .. && git add webui/package.json webui/package-lock.json webui/tsconfig.json webui/.umirc.ts webui/.gitignore webui/src
git commit -m "feat(web): 初始化webui项目骨架与路由结构"
```
...
**Files:**
- Create: `webui/src/types/common.ts`
- Create: `webui/src/types/user.ts`
- Create: `webui/src/types/anime.ts`
- Create: `webui/src/types/watchlist.ts`
- Create: `webui/src/types/review.ts`
- Create: `webui/src/services/user.ts`
- Create: `webui/src/services/anime.ts`
- Create: `webui/src/services/watchlist.ts`
- Create: `webui/src/services/review.ts`
...
```bash
git add webui/src/types webui/src/services
...
**Files:**
- Modify: `webui/src/wrappers/auth.tsx`（替换 Task1 占位实现）
- Modify: `webui/src/layouts/index.tsx`（替换 Task1 占位实现）
...
```bash
git add webui/src/wrappers/auth.tsx webui/src/layouts/index.tsx
...
**Files:**
- Modify: `webui/src/pages/Login/index.tsx`（替换 Task1 占位实现）
...
```bash
git add webui/src/pages/Login/index.tsx
...
**Files:**
- Modify: `webui/src/pages/Register/index.tsx`（替换 Task1 占位实现）
...
```bash
git add webui/src/pages/Register/index.tsx
...
**Files:**
- Modify: `webui/src/pages/AnimeSearch/index.tsx`（替换 Task1 占位实现）
...
```bash
git add webui/src/pages/AnimeSearch/index.tsx
...
**Files:**
- Create: `webui/src/components/ReviewFormModal.tsx`
...
```bash
git add webui/src/components/ReviewFormModal.tsx
...
**Files:**
- Modify: `webui/src/pages/AnimeDetail/index.tsx`（替换 Task1 占位实现）
...
```bash
git add webui/src/pages/AnimeDetail/index.tsx
...
**Files:**
- Modify: `webui/src/pages/Watchlist/index.tsx`（替换 Task1 占位实现）
...
```bash
git add webui/src/pages/Watchlist/index.tsx
...
**Files:**
- Modify: `webui/src/pages/MyReviews/index.tsx`（替换 Task1 占位实现）

**Interfaces:**
- Consumes: `listMyReviews`（Task 2 `@/services/review`）、`ReviewFormModal`（Task 8）
- Produces: 无（叶子页面，webui 全部页面实现完成）
...
```bash
git add webui/src/pages/MyReviews/index.tsx
...
Run: `cd webui && npm run build`
Expected: 退出码为 0，无 TypeScript 报错，`webui/dist/` 生成完整
